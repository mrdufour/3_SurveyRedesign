...
wb_data_2006_2019 <- read.csv("1_data/WB/wb_hacdat_2006_2019.csv") %>%  st_as_sf(coords = c("Lon_M", "Lat_M"), crs = 4326)
...
map <- ggplot()+
  ...
  geom_sf(data = wb_strata, aes(fill = STRATUM), col = "black", show.legend = "polygon", alpha = 0.5)+
  theme_minimal()+
...
## map 2006-2019 data
map +
  geom_sf(data = wb_data_2006_2019, aes(size=NperHa), pch=16, col = "black", alpha = 0.15) +
  facet_wrap(~Year, ncol=4)

ggsave("4_effort_allocation/wb_historical_data_strata.png", width = 6, height = 5, units = "in", dpi = 400)


## intersect data and strata
wb_data_2006_2019 <- st_intersection(wb_data_2006_2019,wb_strata) 
wb_data_2006_2019 <- select(wb_data_2006_2019,-X,-basin)
write.csv(wb_data_2006_2019, "4_effort_allocation/wb_historical_data_strata.csv")
...
STRATUM_Year_sd <- aggregate(NperHa ~ STRATUM + Year, data=wb_data_2006_2019, FUN="sd")
mean_STRATUM_sd <- aggregate(NperHa ~ STRATUM, data=STRATUM_Year_sd, FUN="mean")

alloc <- left_join(mean_STRATUM_sd,wb_strata[,c(2,4)], by="STRATUM") %>% select(-geometry)
...
write.csv(alloc, "4_effort_allocation/wb_effort_allocation.csv")
